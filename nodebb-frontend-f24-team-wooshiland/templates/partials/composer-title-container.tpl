<div class="title-container align-items-center gap-2 d-flex">
	...
	<div class="{{{ if !template.compose }}}d-none d-md-flex{{{ else }}}d-flex{{{ end }}} action-bar gap-1 align-items-center">
		...
		<!-- Source: ChatGPT - Hidden input to store the anonymous field value -->
		<input type="hidden" id="anonymousInput" value="false" />

		<!-- Source: ChatGPT - Post Anonymously Button with Clicked/Unclicked State -->
		<button id="anonymousBtn" class="btn btn-sm btn-outline-secondary fw-bold" data-clicked="false" onclick="toggleButton()">
			<i class="fa fa-user-secret"></i> <span class="d-none d-md-inline">Post Anonymously</span>
		</button>
	</div>
</div>
...
function toggleButton() {
    const button = document.getElementById('anonymousBtn');
    const anonymousInput = document.getElementById('anonymousInput');  // Hidden input for anonymous field
    ...
    if (isClicked) {
        ...
        button.innerHTML = '<i class="fa fa-user-secret"></i> <span class="d-none d-md-inline">Post Anonymously</span>';
        button.setAttribute('data-clicked', 'false');
        anonymousInput.value = 'false';  // Update the anonymous field to false
    } else {
        // Clicked state (gray)
        ...
        anonymousInput.value = 'true';  // Update the anonymous field to true
    }
